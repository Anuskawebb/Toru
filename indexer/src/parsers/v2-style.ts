...
import type { RawEvent, RawSwap, DecodedV2Swap, ParseContext, Dex } from '../types/index.js';
...
export function isV2Swap(event: RawEvent): boolean {
  return (
...
interface V2StyleConfig {
  dex: Dex;
  factories: readonly string[];
}

export function createV2StyleParser(config: V2StyleConfig): EventParser {
  ...
  return {
    ...
    async parse(event: RawEvent, context: ParseContext): Promise<RawSwap | null> {
      ...
      try {
        ...
        logger.debug('V2 style factory check failed/reverted', {
      ...
      try {
        ...
        logger.warn('V2 style decode failed', {
          txHash: event.txHash,
          error:  err instanceof Error ? err.message : String(err),
        });
        return null;
